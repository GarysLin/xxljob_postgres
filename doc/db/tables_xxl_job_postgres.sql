...
CREATE SCHEMA IF NOT EXISTS xxl_job;
CREATE USER xxljob_user WITH PASSWORD 'Aa111111';

-- 建立 xxljob_readwrite 角色
CREATE ROLE xxljob_readwrite;
GRANT CONNECT ON DATABASE postgres TO xxljob_readwrite;
GRANT USAGE, CREATE ON SCHEMA xxl_job TO xxljob_readwrite;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA xxl_job TO xxljob_readwrite;
ALTER DEFAULT PRIVILEGES IN SCHEMA xxl_job GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO xxljob_readwrite;
GRANT USAGE ON ALL SEQUENCES IN SCHEMA xxl_job TO xxljob_readwrite;
ALTER DEFAULT PRIVILEGES IN SCHEMA xxl_job GRANT USAGE ON SEQUENCES TO xxljob_readwrite;

-- 對使用者授予權限
GRANT xxljob_readwrite TO xxljob_user;
...
SELECT setval('xxl_job."xxl_job_group_id_seq"', 2, true);
...
SELECT setval('xxl_job."xxl_job_info_id_seq"', 2, true);
...
SELECT setval('xxl_job."xxl_job_log_id_seq"', 2, true);
...
SELECT setval('xxl_job."xxl_job_logglue_id_seq"', 2, true);


CREATE SEQUENCE xxl_job."xxl_job_log_report_id_seq"
INCREMENT 1
MINVALUE  1
MAXVALUE 2147483647
START 1
...
SELECT setval('xxl_job."xxl_job_log_report_id_seq"', 2, true);
...
SELECT setval('xxl_job."xxl_job_registry_id_seq"', 2, true);
...
SELECT setval('xxl_job."xxl_job_user_id_seq"', 2, true);
...
CREATE TABLE xxl_job."xxl_job_log" (
  "id" int4 NOT NULL DEFAULT nextval('xxl_job.xxl_job_log_id_seq'::regclass),
  "job_group" int4 NOT NULL,
...
CREATE TABLE xxl_job."xxl_job_log_report" (
  "id" int4 NOT NULL DEFAULT nextval('xxl_job.xxl_job_log_report_id_seq'::regclass),
...
CREATE INDEX "i_trigger_day" ON xxl_job."xxl_job_log_report" USING btree (
  "trigger_day" "pg_catalog"."timestamp_ops" ASC NULLS LAST
);


ALTER TABLE xxl_job."xxl_job_log_report" ADD CONSTRAINT "xxl_job_log_report_pkey" PRIMARY KEY ("id");
